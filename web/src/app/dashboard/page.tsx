  ...
  return <><StudentTopbar subtitle={subtitle} onSignOut={onSignOut} /><main id="main" style={{ padding: '32px 0 80px' }}><div className="container">
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 2fr) minmax(0, 1fr)', gap: 24, marginTop: 24, alignItems: 'start' }}>
      <div className="stack" style={{ gap: 20 }}>
        ...
        <section className="card card-content"><h3 style={{ fontSize: 16 }}>Bir sorun mu var?</h3><p className="muted" style={{ marginTop: 8, fontSize: 14 }}>Onarım talebi, yönetilen takvimi silmeden envanter denetimini öne alır.</p><button className="btn btn-secondary btn-sm" type="button" disabled={!reconcileEligible || reconcileBusy} onClick={() => void reconcile()} style={{ marginTop: 14 }}>{reconcileBusy ? 'Gönderiliyor…' : 'Onarım talebi oluştur'}</button>{!reconcileEligible && <p className="muted" style={{ fontSize: 12, marginTop: 8 }}>İlk senkronizasyon ve yönetilen takvim tamamlandığında kullanılabilir.</p>}{reconcileNotice && <p role="status" style={{ marginTop: 10 }}>{reconcileNotice}</p>}</section>
        <section className="card card-content"><DepartmentColorEditor mode="user" /></section>
      </div>
      <div className="stack" style={{ gap: 20 }}>
        <section className="card card-content"><h3 style={{ fontSize: 15 }}>Akademik profil</h3>{errors.profile ? <p className="error">{errors.profile}</p> : profile ? <div style={{ marginTop: 10 }}><div className="summary-row"><span className="muted">Dönem</span><strong>{profile.classYear}</strong></div><div className="summary-row"><span className="muted">Program dili</span><strong>{PROGRAM_LABELS[profile.programLanguage] ?? profile.programLanguage}</strong></div>{Object.entries(profile.selectors).map(([key, value]) => <div className="summary-row" key={key}><span className="muted">{SELECTOR_LABELS[key] ?? key}</span><strong>{value}</strong></div>)}<p style={{ marginTop: 14 }}><Link className="btn btn-tertiary btn-sm" href="/profile">Profili düzenle</Link></p></div> : <p className="loading-note">Yükleniyor…</p>}</section>
