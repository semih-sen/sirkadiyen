...
| G2-ANATOMY-AUTUMN | 2 | TR+EN | Anatomy practice | 1, 2, 3 | Autumn | `donem-2-tr/2. SINIF SALON GRUP SAATLERİ 2025-2026 GÜZ.docx` | grade2_anatomy_autumn_v1 | Documented | Shared by Turkish and English programs; lessons appear as `Diseksiyon` in the annual program. Converted to `g2-anatomy-autumn.snapshot.json` (ADR-076): two tables of 49 and 42 rows, each row `date / hour / anatomy group`. Deliberately absent from `config/schedule-sources.json`: the document is handed out once a semester and has no published URL, which the catalog requires |
| G2-ANATOMY-SPRING | 2 | TR+EN | Anatomy practice | 1, 2, 3 | Spring | `donem-2-tr/2. SINIF SALON GRUP SAATLERİ 2025-2026  BAHAR.docx` | grade2_anatomy_spring_v1 | Documented | As the autumn document. Converted to `g2-anatomy-spring.snapshot.json`: two tables of 49 and 21 rows. Not catalogued, for the same reason |
| G2-VERTICAL | 2 | TR+EN | Vertical corridor | Dynamic | Annual | `donem-2-tr/Dönem 2 Beceri uygulama takvimi güz.docx`<br>`donem-2-tr/Dönem 2 Beceri Uyg BAHAR Takvim 26.01.2025.docx` | grade2_vertical_corridor_v1 | Documented | Shared by Turkish and English programs; lessons appear as `Uygulama` in the annual program. Both catalogued as `G2-VERTICAL-AUTUMN` and `G2-VERTICAL-SPRING` and converted to `g2-vertical-{autumn,spring}.snapshot.json` (ADR-076). Autumn is one 60x7 table, spring is seven; a row is a dated slot and a column is a skill practice, and this is the table the Grade 2 practice sheet's 95 `*` cells defer to. Student Affairs edits these documents during the year, so they must be re-acquired rather than converted once |
