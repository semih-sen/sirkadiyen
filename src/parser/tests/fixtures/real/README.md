...
| `shared-amphi.snapshot.json` | `SHARED-AMPHI` | the weekly room grid every annual source and the three faculty-practice sources read as a companion (ADR-133, ADR-161). Four worksheets: the week of 31 August - 4 September 2026, the previous week, debris from December 2025, and a lone `16 Eylül 2025 / Salı` title stranded in column AC with no header row under it. 203 assignments over six days, of which 116 are Grade 1-3 and the rest are clinical bookings, seminars and examinations; room columns differ between days of one week, one day title sits in column I and another is a date serial, and cells state their own times that contradict the slot row. Two cells state a Grade 3 faculty practice by cohort (`ÇOCUK SAĞ. A3 UYGULAMA`, `NÖROLOJİ -B7-B6-B5 UYGULAMA`) — on 6 September 2026, before the committed rotation begins, so joining the two fixtures places no room and proves only that nothing moves (ADR-102) |
